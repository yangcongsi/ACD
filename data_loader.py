...
class TrainDataLoader(object):
    ...
    def __init__(self, kn_num):
        ...
        data_file = 'data/Math/exe/dataA_train.json'
    ...
    def next_batch(self):
        ...
        # 对Q矩阵进行干扰
        # for i in random.sample(range(len(input_knowedge_embs)), 128):
        #     for j in range(len(input_knowedge_embs[i])):
        #         if input_knowedge_embs[i][j] == 1:
        #             input_knowedge_embs[i][j] = 0
...
class TestDataLoader(object):
    def __init__(self, kn_num):
        ...
        data_file = 'data/Math/exe/dataB_test.json'
        with open(data_file, encoding='utf8') as i_f:
            self.data = json.load(i_f)
